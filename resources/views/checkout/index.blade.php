<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10" style="font-family: 'Signika', sans-serif;">
        ...
        <div class="rounded-3xl bg-gradient-to-br from-primary-light to-white p-8 animate-fade-in-up">
            <h1 class="text-2xl font-bold text-primary-dark">Checkout</h1>
            <p class="text-sm text-neutral-600">Complete your information to place the order.</p>
        </div>
            ...
            <div class="lg:col-span-2 backdrop-blur-xl bg-white/70 shadow-xl rounded-3xl border border-white/20 p-8">
                <form method="POST" action="{{ route('checkout.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-neutral-700 mb-1">Country</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-neutral-700 mb-1">Shipping method</label>
                            <select name="shipping_method" class="w-full rounded-xl bg-white/70 border border-neutral-300 px-4 py-3 shadow-sm focus:ring-primary-dark/30 focus:border-primary-dark transition" required>
                                <option value="flat">Flat Rate (Free)</option>
                            </select>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="pt-2">
                        <button type="submit" class="inline-flex items-center px-6 py-3 rounded-xl bg-primary-dark text-white font-semibold shadow-lg hover:bg-primary-dark/90 transition">Place Order</button>
                    </div>
                </form>
            </div>
                ...
                <div class="bg-white shadow-xl rounded-3xl p-6">
                    ...
                    <div class="mt-4 space-y-4">
                        ...
                        <div class="flex items-center justify-between text-base">
                            <span class="text-primary-dark font-semibold">Total</span>
                            <span class="text-primary-dark font-semibold">€{{ number_format($total, 2, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
                <a href="{{ route('cart.index') }}" class="inline-flex items-center px-5 py-3 rounded-xl border border-primary-gray text-primary-dark hover:bg-primary-gray transition">Back to Cart</a>
            </div>
        </div>
    </div>
</x-app-layout>

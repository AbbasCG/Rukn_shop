    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow p-8 space-y-8">
                <div class="space-y-2">
                    <h1 class="text-2xl font-semibold text-primary-dark">{{ __('My Profile') }}</h1>
                    <p class="text-sm text-neutral-600">{{ __('Manage your personal information and account settings.') }}</p>
                </div>
                ...
                <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="space-y-2">
                            <p class="text-sm text-neutral-700">{{ __('Upload a square image (JPG, PNG, WEBP). Max 5 MB.') }}</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="name">{{ __('Full name') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="email">{{ __('Email address') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="customer_type">{{ __('Customer type') }}</label>
                            <select id="customer_type" name="customer_type" class="w-full rounded-lg border border-neutral-300 px-4 py-2.5 text-sm focus:ring-primary-dark/20 focus:border-primary-dark transition">
                                <option value="" @selected(old('customer_type', $user->customer_type) === null)>{{ __('Select type') }}</option>
                                <option value="personal" @selected(old('customer_type', $user->customer_type) === 'personal')>{{ __('Personal') }}</option>
                                <option value="business" @selected(old('customer_type', $user->customer_type) === 'business')>{{ __('Business') }}</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="address_line1">{{ __('Address line 1') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="address_line2">{{ __('Address line 2') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="postal_code">{{ __('Postal code') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="city">{{ __('City') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-primary-dark mb-1" for="country">{{ __('Country') }}</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2 bg-primary-gray rounded-xl px-4 py-4 border border-neutral-200">
                            <h3 class="text-sm font-semibold text-primary-dark mb-3">{{ __('Account preferences') }}</h3>
                            <div class="flex items-center gap-3">
                                ...
                                <label for="newsletter_opt_in" class="text-sm text-neutral-700">{{ __('I want to receive news and offers') }}</label>
                            </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                        <button type="submit" class="inline-flex items-center px-6 py-2.5 bg-primary-dark text-white font-semibold rounded-lg hover:bg-primary-dark/90 transition">{{ __('Save changes') }}</button>
                        <a href="{{ route('profile.edit') }}" class="inline-flex items-center px-4 py-2 border border-neutral-300 text-sm font-medium rounded-lg text-neutral-700 hover:border-primary-dark hover:text-primary-dark transition">{{ __('Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="max-w-5xl mx-auto mt-8 grid gap-6 md:grid-cols-2">
            <div class="bg-white rounded-2xl shadow p-6 space-y-6">
                <div>
                    <h3 class="text-lg font-semibold text-primary-dark">{{ __('Update Password') }}</h3>
                    <p class="text-sm text-neutral-600">{{ __('Keep your account secure with a strong password.') }}</p>
                </div>
                <form method="post" action="{{ route('password.update') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="update_password_current_password" class="block text-sm font-medium text-primary-dark mb-1">{{ __('Current password') }}</label>
                    ...
                    <div>
                        <label for="update_password_password" class="block text-sm font-medium text-primary-dark mb-1">{{ __('New password') }}</label>
                    ...
                    <div>
                        <label for="update_password_password_confirmation" class="block text-sm font-medium text-primary-dark mb-1">{{ __('Confirm password') }}</label>
                    ...
                    <div class="flex items-center gap-3">
                        <button class="inline-flex items-center px-5 py-2.5 bg-primary-dark text-white text-sm font-semibold rounded-lg hover:bg-primary-dark/90 transition">{{ __('Save password') }}</button>
                        @if (session('status') === 'password-updated')
                            <span class="text-sm text-emerald-700">{{ __('Saved.') }}</span>
                        @endif
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow p-6 space-y-4">
                <div>
                    <h3 class="text-lg font-semibold text-primary-dark">{{ __('Delete Account') }}</h3>
                    <p class="text-sm text-neutral-600">{{ __('This action is permanent. Please confirm with your password.') }}</p>
                </div>
                <x-danger-button
                    x-data=""
                ...
                <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
                    <form method="post" action="{{ route('profile.destroy') }}" class="p-6 space-y-4">
                        ...
                        <h4 class="text-lg font-semibold text-primary-dark">{{ __('Are you sure?') }}</h4>
                        <p class="text-sm text-neutral-600">{{ __('Once deleted, your data cannot be recovered.') }}</p>

                        <div>
                            <label for="password" class="block text-sm font-medium text-primary-dark mb-1">{{ __('Password') }}</label>
